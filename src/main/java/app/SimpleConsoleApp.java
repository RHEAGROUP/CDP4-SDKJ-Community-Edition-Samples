...
public class SimpleConsoleApp {
  ...
  private void postPossibleFiniteStateList() throws TransactionException {
    ...
    if (iteration.isPresent()) {
      var iterationClone = iteration.get().clone(false);
      var pfs1 = new PossibleFiniteState(UUID.randomUUID(), this.session.getAssembler().getCache(),
          this.uri);
      pfs1.setName("state1");
      pfs1.setShortName("s1");

      var pfs2 = new PossibleFiniteState(UUID.randomUUID(), this.session.getAssembler().getCache(),
          this.uri);
      pfs2.setName("state2");
  ...
  private void postParameter() throws TransactionException {
    ...
    if (iteration.isPresent()) {
      ...
      var parameter = new Parameter(UUID.randomUUID(), this.session.getAssembler().getCache(),
          this.uri);
  ...
  private void restore()
    ...
    httpClientBuilder.setDefaultHeaders(Collections.singletonList(
        new BasicHeader(HttpHeaders.AUTHORIZATION, "Basic " + Base64.getEncoder()
            .encodeToString(String.format("%s:%s", this.credentials.getUserName(),
                this.credentials.getPassword()).getBytes(StandardCharsets.US_ASCII)))
    ));
  ...
  /**
   * Removes the first found {@link Parameter} from the first found {@link ElementDefinition}
   */
  ...
  private void getIteration() {
    ...
    var model = new EngineeringModel(engineeringModelIid, this.session.getAssembler().getCache(),
        this.uri);
  ...
  private void postPerson() throws TransactionException {
    ...
    var email1 = new EmailAddress(UUID.randomUUID(), this.session.getAssembler().getCache(),
        this.uri);
    ...
    var email2 = new EmailAddress(UUID.randomUUID(), this.session.getAssembler().getCache(),
        this.uri);
  ...
  private void printCacheSize() {
    System.out
        .println(
            this.session.getAssembler().getCache().size() + " objects currently in the cache.");
  }
  ...
  private void printCommands() {
    ...
    System.out.println(
        POST_PFSL
            + " - posts a predefined PossibleFiniteStateList with 2 PossibleFiniteStates");
    System.out.println(
        POST_PFSL_REORDER
            + " - reorders(rotates in this particular case) states in the created predefined PossibleFiniteStateList (post_pfsl)");
    System.out
        .println(REMOVE_PARAMETER + " - removes a predefined Parameter of ElementDefinition");
    this.printSeparator();
  }
